...
class Game(ShowBase):
    ...
    def playerMovement(self, task):
        ...
        if self.keyMap["jump"] == True: speed += Vec3(0, 0, 0.05)
        # actually pushes player
        self.playerNode.setPos(self.playerNode, speed)
        # makes jumping work only if i am pressing space and the player is touching something
        canJump = self.bulletWorld.contactTest(self.playerNode.node()).getNumContacts()
        if canJump != 0: self.updateKeyMap("jump", False)
        # redos task
        return task.cont
    ...
    def Play(self):
        ...
        self.boxingRing, self.boxingRingNode = self.modelHBMakeRender("Models/BoxingRing.glb", 0, 1, (0, 0, -2), False)
        self.boxingRing.reparentTo(self.boxingRingNode)
        # makes player mesh and model
        self.player, self.playerNode = self.modelHBMakeRender("Models/Player.glb", 1, 1, (0, 0, 0), True)
